...
export default function GiftFinderPage() {
  ...
  return (
    <main className="max-w-3xl mx-auto px-4 py-10">
      <h1 className="text-3xl font-bold mb-4">Gift Finder</h1>
      <form onSubmit={onSubmit} className="grid gap-3 rounded-2xl border p-4">
        <label className="grid gap-1">
          <span>Vesile/Occasion</span>
        ...
        <label className="grid gap-1">
          <span>İlgi alanları (virgülle)</span>
          <input value={interests} onChange={(e) => setInterests(e.target.value)} className="border rounded-xl p-2" />
        </label>
        <button className="mt-2 px-4 py-2 rounded-2xl border w-fit">Öneri Al</button>
      </form>

      <div className="mt-6 grid gap-4">
        {results.map((r) => (
          <AffiliateCard key={r.id} item={r} />
        ))}
      </div>
    </main>
  );
